...
jInteger QueryAddition::AdditionInputBody::AdditionInput::getOperator()
{
	return m_Operator;
}

int QueryAddition::AdditionInputBody::AdditionInput::setOperator(jInteger value)
{
	m_Operator = value;
	setParentPresenceVector();
	return 0;
...
void QueryAddition::AdditionInputBody::AdditionInput::encode(unsigned char *bytes)
{
	...
	pos += sizeof(jUnsignedInteger);
	jInteger m_OperatorTemp;
	
	m_OperatorTemp = JSIDL_v_1_0::correctEndianness(m_Operator);
	memcpy(bytes + pos, &m_OperatorTemp, sizeof(jInteger));
	pos += sizeof(jInteger);
}
...
void QueryAddition::AdditionInputBody::AdditionInput::decode(const unsigned char *bytes)
{
	...
	memcpy(&m_OperatorTemp, bytes + pos, sizeof(jInteger));
	m_Operator = JSIDL_v_1_0::correctEndianness(m_OperatorTemp);
	pos += sizeof(jInteger);
}
...
bool QueryAddition::AdditionInputBody::AdditionInput::operator==(const AdditionInput &value) const
{
	...
	if (m_Operator != value.m_Operator)
	{
		return false;
	}
